#!/usr/bin/env python3
"""Split the hero artwork into real depth layers so its geometry can move.

An overlay copy of the painting's strokes can only ever *ghost* when it moves —
the original strokes stay behind. Real parallax needs the pixels themselves
separated:

* the strokes are cut out of the canvas and inpainted away, leaving a clean
  ground layer;
* those stroke pixels are written to two transparent layers, split by the depth
  the extractor assigned (mid-ground outlines vs. front-most fine ink).

The layers line up exactly at rest, so with no motion the composition is
unchanged; when they drift, the painting's own lines move. Heavy structures
(thick bars, the big rings) deliberately stay in the ground: cutting them would
smear the canvas where they cross colour boundaries.

Outputs land in ``static/site/hero/`` as content-hashed WebP files plus
``layers.json``. After regenerating, update the ``--hero-ground/mid/front``
variables in ``static/site/css/tokens.css`` if the digest changed.

Usage:
    python3 scripts/build_hero_layers.py [width]
"""
...
def main() -> None:
    target_width = int(sys.argv[1]) if len(sys.argv) > 1 else DEFAULT_WIDTH
    ...
    total = sum(
        (OUTPUT_DIR / Path(str(manifest[key])).name).stat().st_size for key in ("ground", "mid", "front")
    )
    print(json.dumps(manifest, ensure_ascii=False, indent=2))
    print(f"frame {width}x{height} · layers total {total / 1024:.0f} KiB")
    print(
        "stroke coverage: "
